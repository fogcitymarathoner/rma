...
class WebTest(WebTest):
    ...
    def test_viewer_can_see_returned_items(self):
        ...
        url = reverse('view_part_to_move', args=(), kwargs={'id': 1})
        resp = self.app.get(url).maybe_follow()
        form = resp.forms['move-part']
        form['part-1'].checked = True

        form['destination_part'].select(text="new part2-None")
        resp = form.submit('submit').maybe_follow()

        form = resp.forms['confirm-parts-move']
        form['part-1'].checked = True
        resp = form.submit('submit').maybe_follow()
        ...
        h1s = page.findAll('h1')
        self.assertEqual('View Part Details', h1s[0].text)
