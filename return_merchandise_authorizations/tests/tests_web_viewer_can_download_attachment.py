...
class WebTest(WebTest):
    ...
    def test_viewer_download_rma_attachment(self):
        """
        Test that viewer can download rma attachement
        """
        ...
        url = reverse('download_rma_attachment', args=(), kwargs={'id': 1})
        resp = self.app.get(url).maybe_follow()
        page = bs(resp.content)
        ps = page.findAll('p')
        self.assertEqual('these are the file contents!', ps[0].text)
