...
class WebTest(WebTest):
    ...
    def test_user_edit_rma(self):
        ...
        print page
        form = resp.forms['edit-rma']
        form['customer'] = str(self.new_customer_site)
        form['reference_number'] = '5678'
        form['last_modified_on'] = '2008-03-09'

        resp = form.submit('Save RMA').maybe_follow()

        page = bs(resp.content)
        print page
        self.assertEqual('\nLast Modified on March 9, 2008\n', page.find('div', {'id':'last-modified-on'}).text)
        pat = '^[\s]*5678[\s]*'
        refnum = page.find('div', {'id':'rma=reference-number'}).text
        self.assertTrue(re.search(pat, refnum))
