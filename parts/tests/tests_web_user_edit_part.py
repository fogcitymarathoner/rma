...
class WebTest(WebTest):
    ...
    def test_user_edit_part(self):
        """
        Test that user can edit existing part
        """
        ...
        url = reverse('edit_part', args=(), kwargs={'id': 1})

        resp = self.app.get(url).maybe_follow()

        page = bs(resp.content)
        form = resp.forms['edit-part']

        form['description'] = 'new part changed'

        resp = form.submit('Save Part').follow()
        page = bs(resp.content)


        title = page.findAll('div', {'class':'page-header'})
        self.assertTrue(title[0].text, 'Edit Part Details')



    def test_user_edit_part_ops(self):
        """
        Test that user can edit existing part 'ops'
        """
        ...
        url = reverse('edit_part_ops', args=(), kwargs={'id': 1})
